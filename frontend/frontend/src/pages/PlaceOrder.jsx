...
const PlaceOrder = () => {
    ...
    const initPay = async (order) => {
        try { await loadRazorpay(); } catch (e) {
            toast.error('Unable to load payment gateway');
            return;
        }

        const options = {
            key: import.meta.env.VITE_RAZORPAY_KEY_ID || 'rzp_live_RL2wqTlyLM6QB9',
            amount: order.amount,
        ...
        try {
            ...
            switch (method) {
                ...
                case 'razorpay':
                    ...
                    if (responseRazorpay.data?.success && responseRazorpay.data?.order?.id) {
